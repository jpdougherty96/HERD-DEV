...
alter table "public"."profiles" add column "last_unread_email_at" timestamp with time zone;

alter table "public"."classes" add constraint "classes_end_date_check" CHECK ((end_date >= start_date)) not valid;

alter table "public"."classes" validate constraint "classes_end_date_check";
...
CREATE TRIGGER classes_sync_end_date BEFORE INSERT OR UPDATE OF start_date, number_of_days ON public.classes FOR EACH ROW EXECUTE FUNCTION trg_sync_end_date();

CREATE TRIGGER trg_increment_unread AFTER INSERT ON public.messages FOR EACH ROW EXECUTE FUNCTION increment_unread_count();
