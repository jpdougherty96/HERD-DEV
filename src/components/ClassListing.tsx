...
export function ClassListing({
  ...
  const ClassCard = ({ cls }: { cls: Class }) => {
    ...
    const ratingCount =
      cls.hostRatingCount ?? cls.hostProfile?.ratingCount ?? 0;
    const averageRating = (() => {
      if (typeof cls.hostRatingAverage === "number")
        return cls.hostRatingAverage;
    ...
    return (
      <Card
        className="bg-[#ffffff] border-[#a8b892] shadow-lg hover:shadow-xl transition-all cursor-pointer overflow-hidden"
      ...
      >
        ...
        <CardHeader className="pb-3">
          <CardTitle className="text-lg text-[#2d3d1f] leading-tight">
            {cls.title}
          </CardTitle>
        </CardHeader>

        <CardContent className="space-y-3">
          <p className="text-[#3c4f21] text-sm line-clamp-2">
            {cls.shortSummary}
          </p>
          ...
          <div className="pt-2 border-t border-[#a8b892] space-y-1">
            ...
            {hasRating ? (
              <div className="flex items-center gap-2 text-sm text-[#3c4f21]">
                ...
                <span>
                  {averageRating!.toFixed(1)} ({ratingCount} review
                  {ratingCount === 1 ? "" : "s"})
                </span>
              </div>
            ) : (
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-[#2d3d1f] mb-2">
              Homesteading Classes
            </h1>
            <p className="text-[#3c4f21]">
              Discover hands-on learning opportunities in your community
            </p>
          </div>
        ...
        <Card className="bg-[#ffffff] border-[#a8b892] p-12 text-center">
          <div className="space-y-4">
            ...
            >
              <Plus className="w-4 h-4 mr-2" />
              {!user
                ? "Sign In to Create Class"
                : !user.stripeConnected
                  ? "Setup Stripe to Create Class"
